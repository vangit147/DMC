...
/*FUNCTION**********************************************************************
 * Function Name : CLOCK_SYS_GetOtherSimClockFreq
...
/*FUNCTION**********************************************************************
 * Function Name : CLOCK_DRV_GetSysClockConfig
...
static void CLOCK_DRV_GetSysClockConfig(scg_system_clock_mode_t sysClockMode, scg_system_clock_config_t *sysClkConfig_ptr)
{
    ...
    bool getClockFreqState;

    if(CLOCK_SYS_GetFircFreq() != 0U)
    {
        getClockFreqState = true;
        (*sysClkConfig_ptr).src     = SCG_SYSTEM_CLOCK_SRC_FIRC;
        (*sysClkConfig_ptr).divCore = defSysClk[TMP_FIRC_CLK][TMP_SYS_DIV];
        (*sysClkConfig_ptr).divBus  = defSysClk[TMP_FIRC_CLK][TMP_BUS_DIV];
        (*sysClkConfig_ptr).divSlow = defSysClk[TMP_FIRC_CLK][TMP_SLOW_DIV];
    }
#if FEATURE_HAS_SPLL_CLK
    else if(CLOCK_SYS_GetSysPllFreq() != 0U)
    {
        ...
        (*sysClkConfig_ptr).divCore = defSysClk[TMP_SPLL_CLK][TMP_SYS_DIV];
        (*sysClkConfig_ptr).divBus  = defSysClk[TMP_SPLL_CLK][TMP_BUS_DIV];
        (*sysClkConfig_ptr).divSlow = defSysClk[TMP_SPLL_CLK][TMP_SLOW_DIV];
    }
#endif
    else if(CLOCK_SYS_GetSysOscFreq() != 0U)
    {
        ...
        (*sysClkConfig_ptr).divCore = defSysClk[TMP_SOSC_CLK][TMP_SYS_DIV];
        (*sysClkConfig_ptr).divBus  = defSysClk[TMP_SOSC_CLK][TMP_BUS_DIV];
        (*sysClkConfig_ptr).divSlow = defSysClk[TMP_SOSC_CLK][TMP_SLOW_DIV];
    }
    else
    {
        getClockFreqState = false;
    }
    if(getClockFreqState == false)
    {
        if(CLOCK_SYS_GetSircFreq() != 0U)
        {
            ...
            {
                (*sysClkConfig_ptr).divCore = FEATURE_VLPR_SYS_CLK;
                (*sysClkConfig_ptr).divBus  = FEATURE_VLPR_BUS_CLK;
                (*sysClkConfig_ptr).divSlow = FEATURE_VLPR_SLOW_CLK;
            }
            else
            {
                (*sysClkConfig_ptr).divCore = defSysClk[TMP_SIRC_CLK][TMP_SYS_DIV];
                (*sysClkConfig_ptr).divBus  = defSysClk[TMP_SIRC_CLK][TMP_BUS_DIV];
                (*sysClkConfig_ptr).divSlow = defSysClk[TMP_SIRC_CLK][TMP_SLOW_DIV];
            }
        }
        else
        {
...
/*FUNCTION**********************************************************************
 * Function Name : SIM_CheckPeripheralClockGate
 * Description   : this func used check is perepheral clock .
 * END**************************************************************************/
static status_t SIM_CheckPeripheralClockGate(clock_names_t clockName)
{
    status_t returnCode = STATUS_SUCCESS;
