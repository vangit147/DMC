                                  ...
                                  bool softwareTrigger)
{
    ...
    else if((state->ftmMode == FTM_MODE_EDGE_ALIGNED_PWM) || (state->ftmMode == FTM_MODE_EDGE_ALIGNED_PWM_AND_INPUT_CAPTURE))
    {
        ftmPeriod = (uint16_t)(ftmPeriod + 1U);
    }
    else
                                 ...
                                 bool softwareTrigger)
{
    ...
    if(STATUS_SUCCESS == retStatus)
    {
        ...
        DEV_ASSERT((FTM_MODE_CEN_ALIGNED_PWM == state->ftmMode) || (FTM_MODE_EDGE_ALIGNED_PWM == state->ftmMode) || (FTM_MODE_EDGE_ALIGNED_PWM_AND_INPUT_CAPTURE == state->ftmMode));
