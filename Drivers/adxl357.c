...
void adxl357_init(void)
{
    ...
    g_norm = 52428.8f;
    printf("ADXL357 g_norm initialized: %.2f\r\n", g_norm);
...
/**
  ...
  * @Parameters : 无
  * @RetValue   : 无
  * @Note       : 从FIFO中批量读取多个样本数据，每个样本9字节（X轴3字节 + Y轴3字节 + Z轴3字节）
  * @Warning    : 注意SPI0总线共享，确保与Flash和IAM20680HT的SPI操作不冲突
  * @CreatedBy  : Assistant
  * @CreatedDate: 2025.01.27
  *******************************************************************************
  */
void adxl357_process_data(void)
{
    ...
    if(entries >= 9) {  // 确保至少有9字节（一个完整样本）
        // 一次性读取所有完整样本
        uint8_t complete_bytes = (entries / 9) * 9; // 计算完整样本的字节数
        uint8_t tx_buffer[complete_bytes + 1]; // 发送缓冲区：命令字节 + 数据字节
        uint8_t rx_buffer[complete_bytes + 1]; // 接收缓冲区：状态字节 + 数据字节
        ...
        // 记录数据处理开始时间
        // uint32_t data_process_start_time = xTaskGetTickCount();

        // 处理这次读取的数据
        for(uint8_t i=0; i<(entries / 9); i++) {
            // 计算当前样本在缓冲区中的起始位置
            uint16_t sample_start = 1 + (i * 9);  // 跳过状态字节，每个样本9字节

            // 提取X轴数据 (前3字节)
            int32_t x_raw = ((int32_t)rx_buffer[sample_start] << 12) |
                        ((int32_t)rx_buffer[sample_start+1] << 4) |
                        ((int32_t)rx_buffer[sample_start+2] >> 4);

            // 提取Y轴数据 (中间3字节)
            int32_t y_raw = ((int32_t)rx_buffer[sample_start+3] << 12) |
                        ((int32_t)rx_buffer[sample_start+4] << 4) |
                        ((int32_t)rx_buffer[sample_start+5] >> 4);

            // 提取Z轴数据 (后3字节)
            int32_t z_raw = ((int32_t)rx_buffer[sample_start+6] << 12) |
                        ((int32_t)rx_buffer[sample_start+7] << 4) |
                        ((int32_t)rx_buffer[sample_start+8] >> 4);

            // 处理20位有符号数据(存储在24位中，最高4位为符号扩展)
            if(x_raw & ADXL357_SIGN_BIT) {  // 检查符号位（第19位）
                x_raw |= 0xFFF00000;  // 符号扩展
            } else {
                x_raw &= ADXL357_DATA_MASK;  // 清除高位的符号扩展
            }

            if(y_raw & ADXL357_SIGN_BIT) {  // 检查符号位（第19位）
                y_raw |= 0xFFF00000;  // 符号扩展
            } else {
                y_raw &= ADXL357_DATA_MASK;  // 清除高位的符号扩展
            }

            if(z_raw & ADXL357_SIGN_BIT) {  // 检查符号位（第19位）
                z_raw |= 0xFFF00000;  // 符号扩展
            } else {
                z_raw &= ADXL357_DATA_MASK;  // 清除高位的符号扩展
            }

            // 过滤全零数据（即使有FIFO清空机制，仍可能偶尔出现）
            if(x_raw == 0 && y_raw == 0 && z_raw == 0) {
                // printf("FIFO Sample %d: Filtering zero data (X=%d, Y=%d, Z=%d)\r\n",
                //        i + 1, x_raw, y_raw, z_raw);
                continue;  // 跳过这个样本，处理下一个
            }

            // 先更新vibration_data中的三轴原始加速度数据字段
            vibration_data.acc_x_raw = x_raw;
            vibration_data.acc_y_raw = y_raw;
            vibration_data.acc_z_raw = z_raw;

            // 计算转换后的加速度数据（g单位）并直接更新到vibration_data结构体
            vibration_data.acc_x_g = (float)vibration_data.acc_x_raw / g_norm;
            vibration_data.acc_y_g = (float)vibration_data.acc_y_raw / g_norm;
            vibration_data.acc_z_g = (float)vibration_data.acc_z_raw / g_norm;

            // 计算原始数据的模长（用于NORM计算）
            // 直接在sqrtf内部进行类型转换，避免整数乘法溢出
            ...
            // 计算当前加速度模长值（基于g单位数据）并更新到vibration_data
            ...
            // 调用Get_Vibration_Data保持原有功能，无需传递参数
            Get_Vibration_Data();

            // 打印每个样本的数据
            // printf("FIFO Sample %d: X=%d, Y=%d, Z=%d | X_g=%.3f, Y_g=%.3f, Z_g=%.3f\r\n",
            //     i + 1, x_raw, y_raw, z_raw, vibration_data.acc_x_g, vibration_data.acc_y_g, vibration_data.acc_z_g);
        }

        // 计算数据处理时间并立即打印
        // uint32_t data_process_end_time = xTaskGetTickCount();
        // uint32_t data_processing_time_ms = (data_process_end_time - data_process_start_time) * portTICK_PERIOD_MS;

        // 打印数据处理时间
        // printf("ADXL357 Data processing: %lu ms\r\n", data_processing_time_ms);
    }

    // 计算并打印总处理时间
    // uint32_t end_time = xTaskGetTickCount();
    // uint32_t processing_time_ms = (end_time - start_time) * portTICK_PERIOD_MS;

    // 每次处理都打印总时间
    // static uint32_t process_count = 0;
    // process_count++;
    // printf("ADXL357[%lu]: Total processing time: %lu ms\r\n", process_count, processing_time_ms);
}
...
static void on_adxl357_data_ready_event(void)
{
    ...
    if(acc_x & ADXL357_SIGN_BIT)
        acc_x |= 0XFFF00000;  // 符号扩展

    // 解析Y轴数据（字节6-8）
    acc_y = transfer_buffer[6] << 12 | transfer_buffer[7] << 4 | transfer_buffer[8] >> 4;
    if(acc_y & ADXL357_SIGN_BIT)
        acc_y |= 0XFFF00000;  // 符号扩展

    // 解析Z轴数据（字节9-11）
    acc_z = transfer_buffer[9] << 12 | transfer_buffer[10] << 4 | transfer_buffer[11] >> 4;
    if(acc_z & ADXL357_SIGN_BIT)
        acc_z |= 0XFFF00000;  // 符号扩展

    // 过滤全零数据（即使有FIFO清空机制，仍可能偶尔出现）
    if(acc_x == 0 && acc_y == 0 && acc_z == 0) {
        printf("ADXL357 Direct Read: Filtering zero data (X=%d, Y=%d, Z=%d)\r\n",
               acc_x, acc_y, acc_z);
        return;  // 直接返回，不处理无效数据
    }

    // 先更新vibration_data中的三轴原始加速度数据字段
    vibration_data.acc_x_raw = acc_x;
    vibration_data.acc_y_raw = acc_y;
    vibration_data.acc_z_raw = acc_z;
    ...
    // 打印调试信息
    // printf("ADXL357 acc_x: %d, acc_y: %d, acc_z: %d\r\n", acc_x, acc_y, acc_z);
    // printf("ADXL357 acc_x_g: %f, acc_y_g: %f, acc_z_g: %f\r\n",
    //        vibration_data.acc_x_g, vibration_data.acc_y_g, vibration_data.acc_z_g);
}
...
void adxl357_task(void* p)
{
    ...
    for(;;)
    {
        ...
        {
            // printf("ADXL357 FIFO_FULL interrupt received!\r\n");

            // 记录任务开始处理时间
            // uint32_t task_start_time = xTaskGetTickCount();

            // FIFO_FULL中断：使用FIFO方式批量读取数据
