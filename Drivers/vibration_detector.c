...
// 子窗口统计变量
float sub_sum = VIBRATION_INIT_ZERO, sub_sum_squared = VIBRATION_INIT_ZERO;
float sub_max = VIBRATION_MAX_INIT_VALUE, sub_min = VIBRATION_MIN_INIT_VALUE;
uint32_t sub_count = 0;
uint32_t variance_over_threshold_count = 0;
uint32_t avg_over_threshold_count = 0;
uint32_t rms_over_threshold_count = 0;
uint32_t peak_over_threshold_count = 0;
...
void vibration_detector_config_init(vibration_detector_t *detector)
{
    ...
    // 峰值检测标志：DISABLED=未启用，ENABLED=已启用
    detector->state.peak_flag = VIBRATION_FLAG_DISABLED;
    // 方差检测标志：DISABLED=未启用，ENABLED=已启用
    ...
    // 平均值检测标志：DISABLED=未启用，ENABLED=已启用
...
void vibration_detector_process(vibration_detector_t *detector)
{
    ...
    // 新增：每20个数据点的统计
    sub_sum += current_value;
    sub_sum_squared += current_value * current_value;
    if(current_value > sub_max) sub_max = current_value;
    if(current_value < sub_min) sub_min = current_value;
    sub_count++;

    // 每20个数据点计算一次方差、均值、RMS、峰值
    if(sub_count >= VIBRATION_SUB_WINDOW_SIZE) {
        detector->data.avg = sub_sum / VIBRATION_SUB_WINDOW_SIZE;
        detector->data.variance = sub_sum_squared / VIBRATION_SUB_WINDOW_SIZE - detector->data.avg * detector->data.avg;
        detector->data.rms = sqrtf(sub_sum_squared / VIBRATION_SUB_WINDOW_SIZE);
        detector->data.peak = sub_max - sub_min;
        ...
        // 重置子窗口统计变量
        sub_sum = VIBRATION_INIT_ZERO;
        sub_sum_squared = VIBRATION_INIT_ZERO;
        sub_max = VIBRATION_MAX_INIT_VALUE;
        sub_min = VIBRATION_MIN_INIT_VALUE;
        sub_count = 0;
    }
    ...
    if(detector->data.count >= VIBRATION_DATA_SIZE) {
        ...
        // 条件1：峰峰值超过阈值（基于80次子窗口计算，70%以上超过阈值）
        if(peak_over_threshold_count >= VIBRATION_SUB_WINDOW_THRESHOLD_COUNT) {
         ...
         // 条件2：方差超过阈值（基于80次子窗口计算，70%以上超过阈值）
        ...
        // 条件4：RMS超过阈值（基于80次子窗口计算，70%以上超过阈值）
        ...
        // 条件5：平均值超过阈值（基于80次子窗口计算，70%以上超过阈值）
        ...
        // 首先确保条件3（delta_flag）为真，然后至少满足指定数量的条件才触发
        if(detector->state.delta_flag == VIBRATION_FLAG_ENABLED && trigger_conditions >= detector->config.min_conditions_count) {
            current_trigger = 1;
        } else {
            current_trigger = 0;
        }
        ...
        // 重置子窗口统计计数器
        variance_over_threshold_count = 0;
        avg_over_threshold_count = 0;
