...
static void prepare_data_transmission(int type) //type: 0.动态数据 1.静态数据
{
    ...
    // 高边数据（实时数据）：将[-180, 180]映射到[0, 360)
    inclination_hs_t hs;
    get_inc_hs(&hs);
    float hs_normalized = (hs.hs_lpf < 0.0f) ? (360.0f + hs.hs_lpf) : hs.hs_lpf;
    pulser_tx_buffer[index++] = 100 * 19 + 8.0f * 100 * (360.0f - hs_normalized) / 360.0f;  // 高边
    pulser_tx_buffer[index++] = 100 * 1;   // HIGH
