...
// 振动检测相关宏定义
#define VIBRATION_SCALE 0.000244140625f                         // IAM-20680 ±8G量程的缩放系数 (1/4096)
...
// 新增：Z轴陀螺仪专用延时补偿器实例
typedef struct {
    float32_t buffer[GZ_DELAY_SIZE];   // Gz延时缓冲区
    uint16_t index;          // 循环索引
...
// 3.动态选择加速度计X/Y的滤波器
    ...
    // 发送highpass_y_outputs数据到VOFA
    //send_highpass_y_outputs_to_vofa();
...
void update_sensor_data(void)
{
    ...
    // 振动检测：计算三轴加速度平方和的根
    // 使用IAM-20680原始数据（已通过21.2Hz低通滤波）进行振动检测
    float vib_ax_g = s_iam_global_raw_data.ax_raw * VIBRATION_SCALE;
    float vib_ay_g = s_iam_global_raw_data.ay_raw * VIBRATION_SCALE;
    float vib_az_g = s_iam_global_raw_data.az_raw * VIBRATION_SCALE;

    // 计算三轴加速度平方和的根
    sensor_data.v_norm_g = sqrt(vib_ax_g * vib_ax_g + vib_ay_g * vib_ay_g + vib_az_g * vib_az_g);
